...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    const [row] = await withDbRetry((database) =>
      database
        .select({
          connection: {
            id: connections.id,
        ...
        .where(and(eq(connections.id, connectionId), eq(connections.followerId, user.id)))
        .limit(1)
    );
    ...
    return jsonError(500, "Unable to load this location.");
  }
}
